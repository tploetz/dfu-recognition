...
parser.add_argument("--dataset_name", type=str, required=False, default="DFU", 
                    help="path to the prediction results")
parser.add_argument("--pred_path", type=str, required=False, default="prediction/dfu/sam_results", 
                    help="path to the prediction results")
parser.add_argument("--gt_path", type=str, required=False, default="data/DFU_split/ulcer/test/labels",
...
print(f"Prediction results directory: {pred_root}")
print(f"Ground truth masks directory: {mask_root}")
print(f"Found {len(mask_name_list)} ground truth mask files")
if os.path.exists(pred_root):
    pred_files = sorted(os.listdir(pred_root))
    print(f"Files in prediction results directory: {len(pred_files)} files")
    if pred_files:
        print(f"Example prediction file: {pred_files[0]}")
else:
    print("Prediction results directory does not exist")
print()

# Use tqdm to add progress bar
progress_bar = tqdm(mask_name_list, desc="Evaluation progress", unit="files")
for i, mask_name in enumerate(progress_bar):
    # Update progress bar display statistics
    ...
    # Build prediction file path - match as long as GT name appears in prediction filename
    base_name = mask_name[:-4]  # Remove .png extension
    
    # Get all files in prediction directory
    pred_files = os.listdir(pred_root) if os.path.exists(pred_root) else []
    
    # Find prediction file containing base_name
    pred_filename = None
    for pred_file in pred_files:
        if base_name in pred_file and pred_file.lower().endswith('.png'):
            pred_filename = pred_file
            break
    
    if pred_filename is None:
        tqdm.write(f"Warning: No corresponding prediction file found, GT: {mask_name}")
        continue
    
    pred_path = os.path.join(pred_root, pred_filename)
    ...
    if mask is None:
        tqdm.write(f"Warning: Cannot read ground truth mask: {mask_path}")
        continue
        
    if pred is None:
        tqdm.write(f"Warning: Cannot read prediction result: {pred_path}")
        continue
    
    if not mask.any() and not pred.any():
        all_black += 1
        continue
...
print(f"All black masks: {all_black} files")
print()
